...
function VpnPolicyList() {
  const [policies, setPolicies] = useState([]);
  const [error, setError] = useState("");

  useEffect(() => {
    fetch("/api/policies/vpn") // Calls the working Go API
      .then((response) => {
        if (!response.ok) {
      ...
      .catch((err) => {
        console.error("Failed to fetch VPN Policies:", err);
        setError("Failed to fetch VPN Policies");
      });
  }, []);
