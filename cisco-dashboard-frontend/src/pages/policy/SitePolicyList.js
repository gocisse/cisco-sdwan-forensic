...
function SitePolicyList() {
  const [policies, setPolicies] = useState([]);
  const [error, setError] = useState("");

  useEffect(() => {
    fetch("/api/policies/sites") // Calls the working Go API
      .then((response) => {
        if (!response.ok) {
      ...
      .catch((err) => {
        console.error("Failed to fetch Site Policies:", err);
        setError("Failed to fetch Site Policies");
      });
  }, []);
