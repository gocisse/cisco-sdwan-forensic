...
function AppPolicyList() {
  ...
  useEffect(() => {
    fetch("/api/policies/app") // Calls your Go API
      .then((response) => {
        if (!response.ok) {
      ...
      .catch((err) => {
        console.error("Failed to fetch App Policy List:", err);
        setError("Failed to fetch App Policy List");
      });
  }, []);

  return (
    <div style={{ padding: "1rem" }}>
      <h1>App Policy List</h1>
