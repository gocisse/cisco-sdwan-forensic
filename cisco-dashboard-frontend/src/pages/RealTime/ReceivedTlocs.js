// src/pages/RealTime/ReceivedTlocs.js
import React, { useState } from 'react';
import { useParams, Link } from 'react-router-dom';
import useApiFetch from '../../hooks/useApiFetch';
import LoadingSpinner from '../../components/LoadingSpinner';

function ReceivedTlocs() {
  ...
  const apiUrl = activeIp ? `/api/tlocs/received/${activeIp}` : null;
  ...
  return (
    <div style={{ padding: '1rem' }}>
      ...
      <h1>Received TLOCs {activeIp && <span style={{ fontSize: '1rem', color: '#666' }}>({activeIp})</span>}</h1>
      ...
      {isLoading && <LoadingSpinner message="Fetching received TLOCs..." />}
