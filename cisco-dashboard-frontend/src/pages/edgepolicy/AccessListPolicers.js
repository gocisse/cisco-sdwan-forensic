// src/pages/edgepolicy/AccessListPolicers.js
...
function AccessListPolicers() {
  ...
  useEffect(() => {
    if (systemIp) {
      setLoading(true);
      fetch(`/api/edgepolicy/accesslistpolicers/${systemIp}`)
        .then((res) => {
          if (!res.ok) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (ipInput.trim()) {
      navigate(`/edgepolicy/accesslistpolicers/${ipInput.trim()}`);
    }
  };

  return (
    <div style={{ padding: "1rem" }}>
      <h1>Access List Policers</h1>
      ...
      {systemIp && (
        <div>
          ...
          {data && data.length > 0 ? (
            ...
            !loading && <p>No Access List Policers data found.</p>
          )}
        </div>
      )}
    </div>
...
export default AccessListPolicers;
