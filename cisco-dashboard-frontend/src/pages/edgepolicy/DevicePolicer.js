// src/pages/edgepolicy/DevicePolicer.js
...
function DevicePolicer() {
  ...
  useEffect(() => {
    if (systemIp) {
      setLoading(true);
      fetch(`/api/edgepolicy/devicepolicer/${systemIp}`)
        .then((res) => {
          if (!res.ok) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (ipInput.trim()) {
      navigate(`/edgepolicy/devicepolicer/${ipInput.trim()}`);
    }
  };

  return (
    <div style={{ padding: "1rem" }}>
      <h1>Device Policer</h1>
            ...
            <table style={{ borderCollapse: "collapse", width: "100%" }}>
              <thead>
                <tr>
                  ...
                  <th style={thStyle}>OOS Action</th>
                  <th style={thStyle}>OOS Bytes</th>
                  <th style={thStyle}>OOS Pkts</th>
              ...
              <tbody>
                {data.map((item, index) => (
                  <tr key={index}>
                    ...
                    <td style={tdStyle}>{item["oos-action"]}</td>
                    <td style={tdStyle}>{item["oos-bytes"]}</td>
                    <td style={tdStyle}>{item["oos-pkts"]}</td>
                    ...
                    <td style={tdStyle}>{item["burst"]}</td>
                    <td style={tdStyle}>{item["vdevice-name"]}</td>
                    <td style={tdStyle}>{item["vdevice-host-name"]}</td>
                    <td style={tdStyle}>{item["direction"]}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ) : (
            !loading && <p>No device policer data found.</p>
          )}
        </div>
      )}
    </div>
